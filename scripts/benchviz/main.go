...
func renderSVG(results []result) string {
	...
	// Find max value (use log scale for wide range)
	var maxNS float64
	...
	for i, r := range results {
		...
		label := r.name
		timeStr := fmtTime(r.ns)

		sb.WriteString(fmt.Sprintf(`<text x="%d" y="%d" class="label">%s</text>`, labelWidth-8, y+barHeight/2, escXML(label)))
		sb.WriteString("\n")
		sb.WriteString(fmt.Sprintf(`<rect class="bar" x="%d" y="%d" width="%d" height="%d" fill="%s"/>`, labelWidth, y, barW, barHeight, color))
		sb.WriteString("\n")
		sb.WriteString(fmt.Sprintf(`<text x="%d" y="%d" class="value"> %s</text>`, labelWidth+barW+4, y+barHeight/2, timeStr))
		sb.WriteString("\n")
	}
